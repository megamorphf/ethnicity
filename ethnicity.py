...
def ethnicity():
    if request.method == 'GET':
        ...
        return render_template('result.html', ethnicity=res['ethnicity'],
...
def ethnicity_search(last_name):
    ...
    if len(result) != 0:
        ethnicitage = {'hispanic': result[0].hispanic,
                       'asian': result[0].asian, 'white': result[0].white,
                       'african': result[0].african}
        top = sorted(ethnicitage.iteritems(), key=operator.itemgetter(1))
        top.reverse()
        res = {'ethnicity': top[0][0],
               'probability': str(int(top[0][1])) + '%'};
    else:
