...
def double_letter_consonant(name):
    ...
    if len(re.findall(r'([B-DF-HJ-NP-TV-Z])\1', name)) > 0:
        value = 1
    else:
        value = 0
    return (key, value)
...
def double_letter_vowel(name):
    ...
    if len(re.findall(r'([AEIOU])\1', name)) > 0:
        value = 1
    else:
        value = 0
    return (key, value)
...
def generate_features(name):
    # features = {'name':name}
    features = {}
    cv_name = consonants_vowels(
        name)  ## need this to calculate some of the features
...
def determine_model(training_data, features_file):
    ...
    print('Out of %d points, %d were incorrectly classified.' % (
    len(test_y), (y_hat != test_y).sum()))
...
def make_prediction(name_dict):
    ...
    return (
    prediction_to_category(prediction), probas_to_categories(probabilities))
...
def probas_to_categories(probas):
    category_probas = {'Asian': round(probas[0][0], 2),
                       'Hispanic': round(probas[0][1], 2),
                       'White': round(probas[0][2], 2)}
